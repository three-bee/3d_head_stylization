...
class TriPlaneGenerator(torch.nn.Module):
    ...
    def synthesis(
        ...
        ready_full_plane=None,
        ready_plane_T=None,
        ready_x_T=None,
        ready_y_T=None,
        T_res=32,
        forward_until_T=False,
        forward_after_T=False,
        forward_full=False,
        generate_background=True,
        make_background_white=False,
        return_only_triplanes=False,
        return_triplanes=False,
        return_xyz_final=False,
        **synthesis_kwargs,
    ):
        assert T_res < 256

        if forward_until_T:
            planes_T, x_T, y_T = self.backbone.synthesis(
                ws,
                T_res=T_res,
                forward_until_T=True,
                forward_after_T=False,
                forward_full=False,
                return_y_T=True,
                update_emas=update_emas,
            ...
            return planes_T, x_T, y_T

        elif forward_after_T:
            planes = self.backbone.synthesis(
                ws,
                T_res=T_res,
                ready_plane_T=ready_plane_T,
                ready_x_T=ready_x_T,
                ready_y_T=ready_y_T,
                forward_until_T=False,
                forward_after_T=True,
                forward_full=False,
                update_emas=update_emas,
                **synthesis_kwargs,
            )
            ...
            if return_only_triplanes:
                return {"triplanes": planes}

            # Perform volume rendering
            feature_samples, depth_samples, weights_samples, xyz_final = self.renderer(
                planes,
                self.decoder,
                ray_origins,
                ray_directions,
            ...
            # Generate Background
            if self.bcg_synthesis and generate_background:
                bcg_image = self.synthesis_background(
                    feature_image, ws, ws_bcg, update_emas=update_emas, **synthesis_kwargs
                )
            ...
            if self.torgb:
                ...
                if make_background_white:
                    assert not generate_background, "Cannot make background white if background is generated"
                    rgb_image = rgb_image * weights_samples + ((1 - weights_samples) * torch.ones_like(weights_samples))
            else:
                rgb_image = feature_image[:, :3]

            # Run SR to get final image
            ...
            return_dict = {
                ...
                "image_mask": weights_samples,
            }
            if return_triplanes:
                return_dict["triplanes"] = planes
            if return_xyz_final:
                return_dict["xyz_final"] = xyz_final
            return return_dict

    def synthesis_background(self, feature_image, ws, ws_bcg=None, update_emas=False, **synthesis_kwargs):
