        ...
        for idx in range(num_layers):
            ...
            if idx == num_layers - 1 and last_activation:
                layer = FullyConnectedLayer(
                    in_features, out_features, activation=last_activation, lr_multiplier=lr_multiplier
                )
            else:
                layer = FullyConnectedLayer(
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, update_emas=False, 
        ...
        ### TODO commented for torch.cuda.amp.autocast
...
class SynthesisNetwork(torch.nn.Module):
    def __init__(
        ...
        **block_kwargs,  # Arguments for SynthesisBlock.
        ...
        self.res_to_ws_idx_mapping = {4:(1), 8:(2,3), 16:(4,5), 32:(6,7), 64:(8,9), 128:(10,11), 256:(12,13,14)}
    ...
    def forward(self, ws, 
        ...
        # Forward for res > T_res, starting from given ready features.
        elif forward_after_T: 
            block_ws = []
            with torch.autograd.profiler.record_function("split_ws"):
                misc.assert_shape(ws, [None, self.num_ws, self.w_dim])
                ws = ws.to(torch.float32)
                w_idx = max(self.res_to_ws_idx_mapping[T_res])
                for res in self.block_resolutions:
                    if res > T_res:
                        block = getattr(self, f"b{res}")
                        block_ws.append(ws.narrow(1, w_idx, block.num_conv + block.num_torgb))
            ...
            y = ready_y_T
            img = ready_plane_T
            for enum_idx, (res, cur_ws) in enumerate(zip(self.block_resolutions[list(self.res_to_ws_idx_mapping).index(T_res)+1:], block_ws)):
                block = getattr(self, f"b{res}")
                x, img  = block(x, img, cur_ws, 
                                ready_y_T=y if enum_idx==0 else None,
                                return_y_T=False,
                                **block_kwargs)
            return img
